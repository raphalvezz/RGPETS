...
// a partir daqui as funções estão fora do código por que apenas um integrante no grupo fez e está fazendo o código e não terá tempo para implementar o resto
        ...
        switch (loginOption) {
            case 1: {
                ...
                if (Donos.find(CPF) != Donos.end()) {
                    menu.menuDono(Donos[CPF], appointmentSystem, Veterinarios);
                } else {
            ...
            case 2: {
                ...
                if (Veterinarios.find(CFMV) != Veterinarios.end() && Veterinarios[CFMV]->verifyPassword(password)) {
                    menu.menuVeterinario(Veterinarios[CFMV]);
                } else {
